...
public sealed class TabulariusDbContext(DbContextOptions<TabulariusDbContext> options) : IdentityDbContext<ApplicationUser>(options)
{
    ...
    protected override void OnModelCreating(ModelBuilder modelBuilder)
    {
        ...
        modelBuilder.Entity<SaftImport>(entity => { entity.Property(item => item.OriginalFileName).HasMaxLength(260).IsRequired(); entity.Property(item => item.ContentHash).HasMaxLength(64).IsFixedLength(); entity.Property(item => item.SaftVersion).HasMaxLength(30).IsRequired(); entity.HasOne(item => item.Dossier).WithMany(item => item.Imports).HasForeignKey(item => item.DossierId).OnDelete(DeleteBehavior.Cascade); var contentHashIndex = entity.HasIndex(item => item.ContentHash).IsUnique(); if (Database.IsSqlServer()) contentHashIndex.HasFilter("[ContentHash] IS NOT NULL"); });
