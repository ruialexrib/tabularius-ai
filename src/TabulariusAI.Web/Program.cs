...
static async Task SeedIdentityAsync(WebApplication application)
{
    ...
    var userManager = scope.ServiceProvider.GetRequiredService<UserManager<ApplicationUser>>(); var dbContext = scope.ServiceProvider.GetRequiredService<TabulariusDbContext>(); var normalizedAdministratorName = userManager.NormalizeName(administratorName); var normalizedAdministratorEmail = userManager.NormalizeEmail(administratorEmail);
    var bootstrapUsers = (await dbContext.Users.Where(user => user.UserName == administratorName || user.NormalizedUserName == normalizedAdministratorName).ToListAsync()).OrderBy(user => user.CreatedAtUtc).ThenBy(user => user.Id).ToList(); ApplicationUser administrator;
