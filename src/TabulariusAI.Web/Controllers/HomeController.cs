...
public sealed class HomeController(ISaftHeaderReader saftHeaderReader, TabulariusDbContext dbContext) : Controller
{
    ...
    public async Task<IActionResult> UploadSaftAsync(IFormFile? saftFile, CancellationToken cancellationToken)
    {
        ...
        try { var contentHash = await CalculateContentHashAsync(saftFile, cancellationToken); if (await dbContext.SaftImports.AsNoTracking().AnyAsync(item => item.ContentHash == contentHash, cancellationToken)) { ModelState.AddModelError("saftFile", "Este ficheiro SAF-T (PT) já foi importado. Selecione um ficheiro diferente."); return View("Import"); } await using var stream = saftFile.OpenReadStream(); var analysis = await saftHeaderReader.ReadAsync(stream, cancellationToken); await PersistImportAsync(saftFile.FileName, contentHash, analysis, cancellationToken); TempData["SaftImportValidation"] = "Ficheiro SAF-T (PT) analisado e importado com sucesso."; return View("Import", analysis); }
    ...
    private async Task PersistImportAsync(string fileName, string contentHash, SaftHeaderViewModel analysis, CancellationToken cancellationToken)
    {
        ...
        foreach (var source in analysis.Transactions) { var persisted=new SaftTransaction { JournalId=source.JournalId, JournalDescription=source.JournalDescription, TransactionId=source.TransactionId, Period=source.Period, TransactionDate=source.TransactionDate, SourceId=source.SourceId, Description=source.Description, DocArchivalNumber=source.DocArchivalNumber, TransactionType=source.TransactionType, GlPostingDate=source.GlPostingDate, CustomerId=source.CustomerId, SupplierId=source.SupplierId }; foreach(var line in source.Lines) persisted.Lines.Add(new SaftTransactionLine { RecordId=line.RecordId, AccountId=line.AccountId, SourceDocumentId=line.SourceDocumentId, SystemEntryDate=line.SystemEntryDate, Description=line.Description, Side=line.Side, Amount=line.Amount }); import.Transactions.Add(persisted); }
        foreach (var source in analysis.SalesInvoices) { var invoice=new SaftSalesInvoice { InvoiceNo=source.InvoiceNo, InvoiceStatus=source.InvoiceStatus, InvoiceDate=source.InvoiceDate, InvoiceType=source.InvoiceType, SourceId=source.SourceId, CustomerId=source.CustomerId, SystemEntryDate=source.SystemEntryDate, TaxPayable=source.TaxPayable, NetTotal=source.NetTotal, GrossTotal=source.GrossTotal }; foreach(var line in source.Lines) invoice.Lines.Add(new SaftSalesInvoiceLine { LineNumber=line.LineNumber, ProductCode=line.ProductCode, ProductDescription=line.ProductDescription, Quantity=line.Quantity, UnitOfMeasure=line.UnitOfMeasure, UnitPrice=line.UnitPrice, TaxPointDate=line.TaxPointDate, TaxType=line.TaxType, TaxCode=line.TaxCode, TaxPercentage=line.TaxPercentage, DebitAmount=line.DebitAmount, CreditAmount=line.CreditAmount }); import.SalesInvoices.Add(invoice); }
        foreach (var source in analysis.StockMovements) { var movement=new SaftStockMovement { DocumentNumber=source.DocumentNumber, DocumentStatus=source.DocumentStatus, MovementDate=source.MovementDate, MovementType=source.MovementType, SourceId=source.SourceId, CustomerId=source.CustomerId, SupplierId=source.SupplierId, SystemEntryDate=source.SystemEntryDate }; foreach(var line in source.Lines) movement.Lines.Add(new SaftStockMovementLine { LineNumber=line.LineNumber, ProductCode=line.ProductCode, ProductDescription=line.ProductDescription, Quantity=line.Quantity, UnitOfMeasure=line.UnitOfMeasure, UnitPrice=line.UnitPrice }); import.StockMovements.Add(movement); }
