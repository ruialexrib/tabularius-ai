...
public sealed class PaymentsTool(TabulariusDbContext db) : IAiTool
{
    public AiToolDefinition Definition { get; }=new("get_payments","Consulta recibos/pagamentos SAF-T persistidos no dossier. Permite filtrar por cliente, referência e intervalo de datas; devolve totais determinísticos em EUR.",new { type="object",properties=new Dictionary<string,object>{{"customerId",new{type="string"}},{"query",new{type="string",description="Referência, tipo ou descrição do pagamento"}},{"fromDate",new{type="string",description="Data inicial YYYY-MM-DD"}},{"toDate",new{type="string",description="Data final YYYY-MM-DD"}},{"limit",new{type="integer",minimum=1,maximum=100}}},required=Array.Empty<string>(),additionalProperties=false });
    public async Task<object> ExecuteAsync(int dossierId,JsonElement args,CancellationToken ct){var ids=db.SaftImports.Where(x=>x.DossierId==dossierId).Select(x=>x.Id);var q=db.SaftPayments.AsNoTracking().Where(x=>ids.Contains(x.SaftImportId));var customer=AiToolArguments.String(args,"customerId");var search=AiToolArguments.String(args,"query");var from=AiToolArguments.Date(args,"fromDate");var to=AiToolArguments.Date(args,"toDate");if(!string.IsNullOrWhiteSpace(customer))q=q.Where(x=>x.CustomerId==customer);if(!string.IsNullOrWhiteSpace(search))q=q.Where(x=>x.PaymentRefNo.Contains(search)||x.PaymentType.Contains(search)||(x.Description!=null&&x.Description.Contains(search)));if(from.HasValue)q=q.Where(x=>x.TransactionDate>=from.Value);if(to.HasValue)q=q.Where(x=>x.TransactionDate<=to.Value);var filtered=q;var rows=await filtered.OrderByDescending(x=>x.TransactionDate).ThenBy(x=>x.PaymentRefNo).Take(AiToolArguments.Limit(args)).Select(x=>new{x.PaymentRefNo,x.TransactionDate,x.PaymentType,x.PaymentStatus,x.CustomerId,x.Description,x.NetTotal,x.TaxPayable,x.GrossTotal,Lines=x.Lines.Select(l=>new{l.LineNumber,l.OriginatingOn,l.InvoiceDate,l.Description,l.DebitAmount,l.CreditAmount}).ToList()}).ToListAsync(ct);return new{CurrencyCode="EUR",CurrencySymbol="€",DocumentLabel="Recibos/pagamentos",Count=rows.Count,FilteredDocuments=await filtered.CountAsync(ct),NetTotal=await filtered.SumAsync(x=>(decimal?)x.NetTotal,ct)??0m,TaxPayable=await filtered.SumAsync(x=>(decimal?)x.TaxPayable,ct)??0m,GrossTotal=await filtered.SumAsync(x=>(decimal?)x.GrossTotal,ct)??0m,Documents=rows};}
}
