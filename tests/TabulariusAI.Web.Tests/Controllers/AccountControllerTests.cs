...
public sealed class AccountControllerTests
{
    ...
    [Fact] public async Task ChangePassword_ValidReplacement_ChangesCredential(){await using var db=new TestDatabase();await using var id=new TestIdentityServices(db.Context);var u=await id.CreateUserAsync("admin","LetMeIn",ApplicationRoles.Administrator);var r=await CreateController(id,u).ChangePassword(new(){CurrentPassword="LetMeIn",NewPassword=StrongPassword,ConfirmPassword=StrongPassword});Assert.IsType<RedirectToActionResult>(r);Assert.True(await id.UserManager.CheckPasswordAsync(u,StrongPassword));}
    /// <summary>Verifies wrong current passwords leave bootstrap credentials unchanged.</summary>
    [Fact] public async Task ChangePassword_WrongCurrentPassword_ReturnsFormError(){await using var db=new TestDatabase();await using var id=new TestIdentityServices(db.Context);var u=await id.CreateUserAsync("admin","LetMeIn",ApplicationRoles.Administrator);var c=CreateController(id,u);Assert.IsType<ViewResult>(await c.ChangePassword(new(){CurrentPassword="wrong",NewPassword=StrongPassword,ConfirmPassword=StrongPassword}));Assert.False(c.ModelState.IsValid);}
    /// <summary>Verifies password-policy failures are translated into form errors.</summary>
    [Fact] public async Task ChangePassword_WeakReplacement_ReturnsFormError(){await using var db=new TestDatabase();await using var id=new TestIdentityServices(db.Context);var u=await id.CreateUserAsync("admin","LetMeIn",ApplicationRoles.Administrator);var c=CreateController(id,u);Assert.IsType<ViewResult>(await c.ChangePassword(new(){CurrentPassword="LetMeIn",NewPassword="weak",ConfirmPassword="weak"}));Assert.False(c.ModelState.IsValid);Assert.True(await id.UserManager.CheckPasswordAsync(u,"LetMeIn"));}
    /// <summary>Verifies invalid password-change model state returns without changing credentials.</summary>
    [Fact] public async Task ChangePassword_InvalidModelState_ReturnsForm(){await using var db=new TestDatabase();await using var id=new TestIdentityServices(db.Context);var u=await id.CreateUserAsync("admin","LetMeIn",ApplicationRoles.Administrator);var c=CreateController(id,u);c.ModelState.AddModelError("NewPassword","invalid");Assert.IsType<ViewResult>(await c.ChangePassword(new(){CurrentPassword="LetMeIn",NewPassword=StrongPassword,ConfirmPassword=StrongPassword}));Assert.True(await id.UserManager.CheckPasswordAsync(u,"LetMeIn"));}
    /// <summary>Verifies password-change POST for anonymous users redirects to login.</summary>
    [Fact] public async Task ChangePassword_PostAnonymous_RedirectsLogin(){await using var db=new TestDatabase();await using var id=new TestIdentityServices(db.Context);var r=Assert.IsType<RedirectToActionResult>(await CreateController(id).ChangePassword(new(){CurrentPassword="x",NewPassword=StrongPassword,ConfirmPassword=StrongPassword}));Assert.Equal("Login",r.ActionName);}
