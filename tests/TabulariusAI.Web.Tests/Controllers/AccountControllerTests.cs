...
public sealed class AccountControllerTests
{
    ...
    /// <summary>Verifies authenticated non-bootstrap users can open the optional password-change form.</summary>
    [Fact] public async Task ChangePassword_Get_NonBootstrapUser_ReturnsForm(){await using var db=new TestDatabase();await using var id=new TestIdentityServices(db.Context);var u=await id.CreateUserAsync("rui",StrongPassword);var c=CreateController(id,u);var model=Assert.IsType<ChangePasswordViewModel>(Assert.IsType<ViewResult>(await c.ChangePassword()).Model);Assert.False(Assert.IsType<bool>(c.ViewData["MandatoryPasswordChange"]));Assert.NotNull(model);}
